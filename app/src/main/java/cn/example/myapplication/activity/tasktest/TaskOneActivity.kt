 ...
 *  description :
 */
class TaskOneActivity: BaseActivity() {
    override fun getLayoutId(): Int {
        return R.layout.activity_task_one
    }

    override fun initOnCreate() {
        taskTv.text = "task standard"
        taskTv.setOnClickListener {
            startActivityForResult(Intent(this, SingleTaskActivity::class.java), 100)
        }
    }

    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        super.onActivityResult(requestCode, resultCode, data)
        Log.e(TAG, "task standard onActivityResult invoke")
    }
}